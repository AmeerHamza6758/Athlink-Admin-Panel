...
const IconInput = ({ placeholder, onchange, stateValue }) => {
    ...
    return (
        <div className='px-4 w-full h-16 border border-[#E2E8F0] rounded-md bg-transparent flex flex-row justify-center items-center'>
            ...
            {
                inputType === 'password' &&
                <FaRegEyeSlash size={25} color='#808D9E' className='w-1/12 cursor-pointer' onClick={handleInputTypeChange}/>
            }
            {
                inputType === 'text' &&
                <FaRegEye size={25} color='#808D9E' className='w-1/12 cursor-pointer' onClick={handleInputTypeChange} />
            }
        </div>
    );
}
